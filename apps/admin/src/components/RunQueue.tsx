...
export function RunQueue({ runs, loading, activeFilter, selectedID, selectionMode, selectedIDs, bulkBusy, onSelect, onToggleSelection, onToggleSelectionMode, onToggleAll, onBulkPublish, onBulkDelete, onRefresh }: Props) {
  ...
  return <div className="border-b border-ink/15 bg-[#eeece4] xl:border-b-0 xl:border-r">
    ...
    <div className="max-h-[520px] overflow-y-auto xl:max-h-[calc(100vh-255px)]">
      ...
      {runs.map((run) => <QueueItem key={run.id} run={run} active={!selectionMode && selectedID === run.id} selectionMode={selectionMode} checked={selectedIDs.has(run.id)} onClick={() => selectionMode ? onToggleSelection(run.id) : onSelect(run.id)} />)}
    </div>
...
function QueueItem({ run, active, selectionMode, checked, onClick }: { run: Run; active: boolean; selectionMode: boolean; checked: boolean; onClick: () => void }) {
  const hasProblem = run.issues.length > 0 || run.state === "failed";
  return <button type="button" onClick={onClick} aria-pressed={selectionMode ? checked : undefined} className={`focus-ring flex w-full items-start gap-3 border-b border-ink/10 px-5 py-4 text-left transition ${checked ? "bg-[#fffdf7]" : active ? "bg-[#fffdf7]" : "hover:bg-white/55"}`}>
