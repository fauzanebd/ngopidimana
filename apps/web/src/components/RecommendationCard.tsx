...
export function RecommendationCard({ place, rank, showDistance, expanded, onExpand }: Props) {
  return <article className="result-card animate-soft-in overflow-hidden rounded-xl border border-ink/15 bg-[#fffdf7]/95 transition duration-200 hover:border-ink/30" style={{ animationDelay: `${Math.min(rank * 25, 250)}ms` }}>
    <div className="grid sm:grid-cols-[122px_1fr]">
      ...
      <div className="p-4 sm:p-5">
        <div className="flex items-start gap-3">
          <div><div className="mb-1 flex flex-wrap items-center gap-2"><h2 className="font-display text-[27px] leading-none">{place.name}</h2><span className="rounded-md bg-[#e8eddc] px-2 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-moss">{Math.round(place.match_score * 100)}% match</span></div><p className="flex items-center gap-1.5 text-xs text-ink/50"><MapPin className="h-3 w-3" /> {place.area}{showDistance ? ` · ${place.distance_km} km` : ""}</p></div>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-ink/10 pt-3 text-xs text-ink/55">
          ...
          <span className="flex items-center gap-1"><Clock3 className="h-3.5 w-3.5" /> {place.open_24_hours ? "Open 24 hours" : `until ${place.open_until}`}</span>
          <span className="flex items-center gap-1"><Gauge className="h-3.5 w-3.5" /> {Math.round(place.result_confidence * 100)}% evidence</span>
        </div>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          <button onClick={onExpand} type="button" className="focus-ring flex items-center gap-1.5 rounded-sm text-xs font-medium text-ink/55 hover:text-ink" aria-expanded={expanded}>Why this fits <ChevronDown className={`h-3.5 w-3.5 transition ${expanded ? "rotate-180" : ""}`} /></button>
          <div className="flex items-center gap-2">
            {place.review_links[0] ? <a className="focus-ring inline-flex h-9 items-center gap-1.5 rounded-lg border border-ink/15 px-3 text-xs font-medium transition hover:bg-cream" href={place.review_links[0].url} target="_blank" rel="noreferrer">Review <ExternalLink className="h-3.5 w-3.5" /></a> : null}
            <a className="focus-ring inline-flex h-9 items-center gap-1.5 rounded-lg bg-moss px-3 text-xs font-semibold text-white transition hover:bg-[#1f3b2a]" href={place.maps_url} target="_blank" rel="noreferrer">Open Maps <ArrowUpRight className="h-3.5 w-3.5" /></a>
          </div>
        </div>
      </div>
    </div>
    {expanded ? <div className="animate-soft-in border-t border-ink/10 bg-[#f1f3e9] px-4 py-4 sm:pl-[142px] sm:pr-5"><div className="grid gap-4 sm:grid-cols-2">
      <div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-ink/45">Score breakdown</p><div className="mt-2 space-y-2">{Object.entries(place.score_components).map(([key, value]) => <div key={key} className="grid grid-cols-[92px_1fr_30px] items-center gap-2 text-[11px] text-ink/55"><span className="capitalize">{key.replace("_", " ")}</span><span className="h-1.5 overflow-hidden rounded-full bg-ink/10"><span className="block h-full rounded-full bg-olive" style={{ width: `${value * 100}%` }} /></span><span className="text-right">{Math.round(value * 100)}</span></div>)}</div></div>
      <div className="text-xs leading-5 text-ink/55"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-ink/45">Evidence note</p><p className="mt-2">{place.evidence_summary}</p>{place.caveats.map((caveat) => <p key={caveat} className="mt-1.5 flex gap-1.5 text-amber-900"><CircleAlert className="mt-0.5 h-3.5 w-3.5 shrink-0" /> {caveat}</p>)}</div>
    </div></div> : null}
  </article>;
}
